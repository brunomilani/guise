...
export function parseGhAuthStatus(text: string): GhAccount[] {
  ...
  for (const raw of lines) {
    ...
    if (login) {
      flush();
      const host = login[1];
      if (/^[a-zA-Z0-9.-]+$/.test(host)) currentHost = host;
      current = {
...
export function getActiveGhAccount(
  accounts: GhAccount[],
  host?: string,
): GhAccount | undefined {
 ...
 */
export function ghSwitch(host: string, user: string): boolean {
  try {
