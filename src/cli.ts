...
import type { GhAccount, RepoChoice, RepoInfo } from "./types.js";
...
function repoProvider(repo: RepoInfo): "github" | "gitlab" | "bitbucket" | "unknown" {
 ...
 */
function inactiveIfUnsupported(repo: RepoInfo, quiet = false): boolean {
  const p = repoProvider(repo);
  if (isSupported(p)) return false;
  if (!quiet) {
...
async function cmdInit(): Promise<number> {
  ...
  if (inactiveIfUnsupported(repo)) return 0;
  // Bitbucket/GitLab have no account CLI; take the SSH-key + identity path.
  if (usesSshModel(repoProvider(repo))) return initSshProvider(repo);
  if (!ghOk) {
  ...
  if (existing && !flags.force && !flags.user) {
    ...
    if (!process.stdin.isTTY) return 0; // non-interactive: keep current
    if (!(await confirm("Re-choose the account?", false))) return 0;
  }
 ...
 */
async function initSshProvider(repo: RepoInfo): Promise<number> {
  const flags = parseFlags(process.argv.slice(3));
  ...
  if (typeof flags.profile === "string" || typeof flags.user === "string") {
    const user =
      typeof flags.user === "string" ? flags.user : getProfile(flags.profile as string)?.githubUser;
    if (!user) {
      out(red(`Unknown profile "${flags.profile}". List them: guise profile list`));
      return 1;
    }
    if (!saveChoice(repo, user, flags)) return 1;
  ...
  if (!process.stdin.isTTY) {
    ...
    const inferred: Record<string, string | boolean> = { user: owner, yes: true };
    if (repo.gitUserEmail) inferred.email = repo.gitUserEmail;
    // saveChoice auto-detects the SSH key for the host when none is passed.
    if (!saveChoice(repo, owner, inferred)) return 1;
  ...
  if (detected) out(dim(`Detected SSH key for ${repo.remote?.rawHost}: ${detected}`));
  const keyAns = (await prompt(`SSH key${detected ? ` [${detected}]` : " (path, optional)"}: `)).trim();
  const sshKey = keyAns || detected;

  const persistFlags: Record<string, string | boolean> = { user };
  if (email) persistFlags.email = email;
  if (sshKey) persistFlags["ssh-key"] = sshKey;
  if (!saveChoice(repo, user, persistFlags)) return 1;
  return applySshAfterInit(repo, persistFlags);
}

/** Offer to apply the SSH key + git identity right after a Bitbucket init. */
async function applySshAfterInit(
  repo: RepoInfo,
  ...
  if (choice.sshKey) setLocalSshKey(repo.path, choice.sshKey);
...
function saveChoice(
  ...
  const choice = setRepoChoice(
    repo.path,
    {
      ...
      provider,
      gitUserName:
        typeof flags.name === "string" ? flags.name : profile?.gitUserName ?? repo.gitUserName,
      gitUserEmail:
  ...
  out(
    `Configured account: ${green(choice.githubUser)} on ${choice.host}` +
...
async function chooseAccount(
  ...
  let selected: GhAccount;
  if (pool.length === 1) {
    if (!(await confirm(`Use "${pool[0].user}" for this repository?`, true))) return null;
    selected = pool[0];
  } else {
...
async function cmdChoose(): Promise<number> {
  const { repo, accounts, ghOk } = loadContext();
  if (inactiveIfUnsupported(repo)) return 0;
  if (usesSshModel(repoProvider(repo))) return initSshProvider(repo);
  if (!ghOk) {
...
async function cmdUse(): Promise<number> {
  const { repo, accounts, ghOk } = loadContext();
  const choice = getRepoChoice(repo.path);
  if (inactiveIfUnsupported(repo)) return 0;
  if (!choice) {
  ...
  if (usesSshModel(repoProvider(repo))) return useSshProvider(repo, choice);
  if (!ghOk) {
    out(red("gh is not installed."));
    return 1;
  }
  ...
  if (!target) {
    out(
      red(
        `The GitHub account configured for this repo is "${choice.githubUser}", ` +
 ...
 */
function useSshProvider(repo: RepoInfo, choice: RepoChoice): number {
  if (choice.sshKey) {
...
async function cmdGitSync(): Promise<number> {
  const { repo } = loadContext();
  if (inactiveIfUnsupported(repo)) return 0;
  const choice = getRepoChoice(repo.path);
  if (!choice) {
  ...
  if (choice.sshKey) out(`  ssh key    = ${choice.sshKey}`);
  out("");
...
function cmdStatus(): number {
  ...
  if (policy) {
    out(bold("Policy:"));
    if (policy.expectedOwner) out(`  expectedOwner: ${policy.expectedOwner}`);
    if (policy.requireGitEmailDomain) out(`  requireGitEmailDomain: ${policy.requireGitEmailDomain}`);
    if (policy.allowedUsers) out(`  allowedUsers: ${policy.allowedUsers.join(", ")}`);
 ...
 */
async function cmdStatusline(): Promise<number> {
  try {
    ...
    if (!isSupported(provider)) return 0; // GitLab/unknown; show nothing
    const choice = getRepoChoice(repo.path);
    const label = choice
 ...
 */
async function cmdHookPreBash(): Promise<number> {
  ...
  if (!isIdentitySensitive(intent)) return 0;

  const bootstrap = getRepoInfo(cwd)!;
  if (!isSupported(repoProvider(bootstrap))) return 0; // GitHub-only; no-op elsewhere
  const choice = getRepoChoice(bootstrap.path);
  if (!choice) return 0; // nothing configured → stay out of the way
  const policy = loadPolicy(bootstrap.path);
  ...
  if (intent.touchesCommit) {
    ...
    if (expectDomain && domain !== expectDomain) {
      process.stderr.write(
        `guise: refusing commit — git email "${email || "(unset)"}" is not @${expectDomain}. ` +
          `Run: guise git-sync\n`,
      );
      return 2;
    }
    if (choice.gitUserEmail && email && email !== choice.gitUserEmail) {
      process.stderr.write(
        `guise: git email "${email}" differs from the chosen account's "${choice.gitUserEmail}". ` +
          `Run: guise git-sync\n`,
      );
      return 2;
    }
  ...
  if (usesSshModel(repoProvider(bootstrap))) {
    if (!intent.touchesPush) return 0;
    if (policy?.enforceBeforePush === false) return 0;
    if (!choice.sshKey) return 0; // relying on the user's global SSH config
    if (!existsSync(expandTilde(choice.sshKey))) {
      process.stderr.write(
        `guise: configured SSH key "${choice.sshKey}" is missing. Run: guise set --ssh-key <path>\n`,
      );
      return 2;
    }
  ...
  if (!wantsRemoteAuth) return 0;
  if (intent.touchesGh && policy?.enforceBeforeGhCommands === false) return 0;
  if (intent.touchesPush && policy?.enforceBeforePush === false) return 0;

  if (!isGhInstalled()) return 0;
  const accounts = listGhAccounts();
  const active = getActiveGhAccount(accounts, choice.host);
  if (active && active.user === choice.githubUser) return 0; // already correct

  const target = findGhAccount(accounts, choice.host, choice.githubUser);
  if (!target) {
    process.stderr.write(
      `guise: account "${choice.githubUser}" for this repo is not authenticated. ` +
